...
- (void)iconClick
{
    ...
    if([[regular getUID] longValue]==[[_detailModel.user objectForKey:@"id"] longLongValue])
    {
    ...
    UITapGestureRecognizer *tap1=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(NullAction:)];
    ...
    NSString *url = [NSString stringWithFormat:@"%@/v1/users/%lld?token=%@",DNS,[[_detailModel.user objectForKey:@"id"] longLongValue],[regular getToken]];
