...
{
    self.navigationItem.titleView=[[ToolManager sharedManager] returnNavView:h_title withmaxwidth:230];
...
-(void)setUserinfo:(NSDictionary *)userinfo
{
    if(_userinfo!=userinfo)
    {
        ...
        _is_server=[[_userinfo objectForKey:@"is_server"] boolValue];
        _uid=[userinfo objectForKey:@"uid"];
        _cell=[_userinfo objectForKey:@"cell"];
        if(_is_server)
        {
//            添加电话拨打
            if(![_cell isEqualToString:@""])
            {
                UIButton *btn=[UIButton buttonWithType:UIButtonTypeCustom];
                btn.frame=CGRectMake(0, 0, 18, 18);
                [btn setImage:[UIImage imageNamed:@"chatview_电话"] forState:UIControlStateNormal];
                [btn addTarget:self action:@selector(call_action) forControlEvents:UIControlEventTouchUpInside];

                UIBarButtonItem *_btn=[[UIBarButtonItem alloc] initWithCustomView:btn];
...
- (NSMutableArray *)dataSource
{
    ...
    for (id _d in _dataSource) {
        if([_d isKindOfClass:[MessageModel class]])
        {
            MessageModel *model=(MessageModel *)_d;

            NSString *head_img=nil;
            if(model.isSender)
            {
               head_img=[[[NSUserDefaults standardUserDefaults] objectForKey:@"userImageurl"] objectForKey:@"image"];

            }else
            {
                head_img=self.friend_head;

            }
            NSURL *url=[NSURL URLWithString:head_img];
            model.headImageURL=url;
        }
    }
...
//        ChatMoreType type = self.isChatGroup == YES ? ChatMoreTypeGroupChat : ChatMoreTypeChat;
        _chatToolBar.moreView = [[DXChatBarMoreView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight + (kIPhoneX?34.f:0.f)), _chatToolBar.frame.size.width, 80) type:ChatMoreTypeGroupChat];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            timeCell.textLabel.font=[regular get_en_Font:11.0f];
...
//            NSObject *obj2 = [self.dataSource objectAtIndex:indexPath.row];
//            CGFloat _height=[EMChatViewCell tableView:tableView heightForRowAtIndexPath:indexPath withObject:(MessageModel *)obj2];
            cell.height=[EMChatViewCell tableView:tableView heightForRowAtIndexPath:indexPath withObject:(MessageModel *)obj];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    text.leftViewMode=YES;
    text.leftView=[UIView new];
...
- (void)moreViewPhotoAction:(DXChatBarMoreView *)moreView
{
    ...
    if ([self.imagePicker.navigationBar respondsToSelector:@selector( setBackgroundImage:forBarMetrics:)]){
    ...
    self.imagePicker.navigationBar.barTintColor =[UIColor colorWithRed:70.0f/255.0f green:195.0f/255.0f blue:247.0f/255.0f alpha:1];
...
//    3    //大功告成
    ...
    self.imagePicker.navigationBar.titleTextAttributes = dict;


    // 弹出照片选择
    self.imagePicker.sourceType = UIImagePickerControllerSourceTypePhotoLibrary;
    self.imagePicker.mediaTypes = @[(NSString *)kUTTypeImage];
//    self.imagePicker.title=@"hh";
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:NSLocalizedString(@"media.timeShort", @"说话时间太短")];
...
- (void)loadMoreMessagesFrom:(long long)timestamp count:(NSInteger)count append:(BOOL)append
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        {
            for (EMMessage *Message in self.messages) {
                if(Message.messageId==((EMMessage*)[messages objectAtIndex:0]).messageId)
                {
                    _is_wangluo=YES;
                    break;
                }
        ...
        if ([messages count] > 0&&!_is_wangluo) {
...
-(void)getHistoryMessage
{
    if(if_refresh)
    {
        ...
        {
            EMMessage *firstMessage = [self.messages firstObject];
            long long min_time=firstMessage.timestamp;
//            msg_id=firstMessage.messageId;
            for (EMMessage *Message in self.messages) {
                if(min_time>=Message.timestamp)
                {
                    min_time=Message.timestamp;
            ...
            if([[dict objectForKey:@"code"] integerValue]==1)
            {
                ...
                if([[dict objectForKey:@"data"] count])
                {
                    ...
                    }else
                    {
                        ...
                        arr= [[arr1 reverseObjectEnumerator] allObjects];
                    }
                    for (int i=0; i<arr.count; i++) {
                        NSDictionary *info=[arr objectAtIndex:i];
                        [self _insertMessageWithStr:info];
                    }
...
- (void)_insertMessageWithStr:(NSDictionary *)dict
{

//    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *docDir = [paths objectAtIndex:0];//本地路径

    NSArray *bodies=[[dict objectForKey:@"payload"] objectForKey:@"bodies"];

    EMMessage *message =nil;

    if([[[bodies firstObject] objectForKey:@"type"] isEqualToString:@"txt"])
    {
        NSString *_msg=[[bodies firstObject] objectForKey:@"msg"];

        EMChatText *chatText = [[EMChatText alloc] initWithText:_msg];

        EMTextMessageBody *textBody = [[EMTextMessageBody alloc] initWithChatObject:chatText];
        message=[[EMMessage alloc] initWithReceiver:_chatter bodies:@[textBody]];
    }else if([[[bodies firstObject] objectForKey:@"type"] isEqualToString:@"img"])
    {

        EMChatImage *img=[[EMChatImage alloc]initWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[[bodies firstObject] objectForKey:@"url"]]] displayName:[[bodies firstObject] objectForKey:@"filename"]];
        EMImageMessageBody *imageBody = [[EMImageMessageBody alloc] initWithImage:img thumbnailImage:img];

        message = [[EMMessage alloc]initWithReceiver:[dict objectForKey:@"to"] bodies:@[imageBody]];


    }else if([[[bodies firstObject] objectForKey:@"type"] isEqualToString:@"loc"])
    {

        EMChatLocation *loc=[[EMChatLocation alloc] initWithLatitude:[[[bodies firstObject] objectForKey:@"lat"] doubleValue] longitude:[[[bodies firstObject] objectForKey:@"lng"] doubleValue] address:[[bodies firstObject] objectForKey:@"addr"]];

        EMLocationMessageBody *textBody=[[EMLocationMessageBody alloc] initWithChatObject:loc];

        message=[[EMMessage alloc] initWithReceiver:_chatter bodies:@[textBody]];

    }else if([[[bodies firstObject] objectForKey:@"type"] isEqualToString:@"audio"])
    {
//        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//        NSString *docDir = [paths objectAtIndex:0];
//        EMChatVoice *chat=[[EMChatVoice alloc]initWithFile:docDir displayName:[[bodies firstObject] objectForKey:@"filename"]];
//        EMChatVoice *chat=[[EMChatVoice alloc]initWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[[bodies firstObject] objectForKey:@"url"]]] displayName:[[bodies firstObject] objectForKey:@"filename"]];
//        chat.fileLength=[[[bodies firstObject] objectForKey:@"file_length"] longValue];
//        chat.localPath=docDir;
//        chat.duration=[[[bodies firstObject] objectForKey:@"length"] integerValue];
//
//        EMVoiceMessageBody *textBody = [[EMVoiceMessageBody alloc] initWithChatObject:chat];
//
//        textBody.attachmentDownloadStatus=EMAttachmentDownloadSuccessed
//            ;
//        message=[[EMMessage alloc] initWithReceiver:_chatter bodies:@[textBody]];
    ...
    NSDictionary *voiceDic = @{EMMessageBodyAttrKeySecret:[[bodies firstObject] objectForKey:@"secret"],
                               ...
                               EMMessageBodyAttrKeyUrl:[[bodies firstObject] objectForKey:@"url"],
                               EMMessageBodyAttrKeyFileLength:[[NSString alloc]initWithFormat:@"%ld",[[[bodies firstObject] objectForKey:@"file_length"] longValue]]};

    EMVoiceMessageBody *voiceBody = [EMVoiceMessageBody voiceMessageBodyFromBodyDict:voiceDic forChatter:_chatter];
//    voiceBody.attachmentDownloadStatus=EMAttachmentDownloading;
    message=[[EMMessage alloc] initWithReceiver:_chatter bodies:@[voiceBody]];

    }

    message.isReadAcked=YES;
    message.messageId=[dict objectForKey:@"msg_id"];
    message.from=[dict objectForKey:@"from"];
    message.to=[dict objectForKey:@"to"];
    message.messageType=eMessageTypeChat;
    message.timestamp=[[dict objectForKey:@"timestamp"] longLongValue];
    message.isRead = YES;
    message.deliveryState = eMessageDeliveryState_Delivered;
    __weak typeof(self) weakSelf = self;
    [weakSelf downloadMessageAttachments:message];
    [self addMessage_history:message];
    [[EaseMob sharedInstance].chatManager insertMessageToDB:message append2Chat:YES];
}

-(void)addMessage_history:(EMMessage *)message
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            {
                NSMutableArray *arr=[[NSMutableArray alloc] initWithArray:weakSelf.dataSource];
