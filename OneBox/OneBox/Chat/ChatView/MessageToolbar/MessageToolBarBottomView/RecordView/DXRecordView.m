...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _textLabel.text = NSLocalizedString(@"message.toolBar.record.upCancel", @"移动到此处，取消发送");
        [self addSubview:_textLabel];
        _textLabel.font = [regular getFont:13.0f];
...
{
    _textLabel.text = NSLocalizedString(@"message.toolBar.record.loosenCancel", @"松开手指，取消发送");
    _textLabel.backgroundColor = [UIColor clearColor];
}
