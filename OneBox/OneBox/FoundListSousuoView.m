...
@interface FoundListSousuoView()<UITextFieldDelegate>

@property (nonatomic, strong) UITextField *textFiled;

@end

@implementation FoundListSousuoView
...
//创建背景
-(void)createBackView{
    UIImageView *backImgView = [UIImageView getImgWithImageStr:@"蒙板"];
...
-(void)createSearchView
{
    ...
    [_textFiled mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(120*_Scale+kStatusBarAndNavigationBarHeight);
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    }else
    {
        //跳转到搜索页面
        if(_sousuoViewBlock){
            _sousuoViewBlock(@"gotoSouSuoView",textField.text);
        }
    }
