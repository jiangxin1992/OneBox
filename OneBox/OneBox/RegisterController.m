...
-(void)UIConfig
{
    ...
    NSArray *titlearr=@[@" 昵 称 ",@" 密 码 ",@" 邮 箱 "];
    CGFloat _y_p=244*_Scale;
    CGFloat _height=50*_Scale;

    for (int i=0; i<titlearr.count; i++) {

        UITextField *textfield=i==0?textfield_nickname:i==1?textfield_password:textfield_email;

        CGRect _rect;
        if(i==1)
        {
            _rect=CGRectMake((ScreenWidth-400*_Scale)/2.0f, _y_p+(_height+15*_Scale)*i, 280*_Scale,_height);
...
//        [put_backView addSubview:textfield];
        ...
        if(i==1)
        {
            ...
            showpass_btn.frame=CGRectMake(CGRectGetMaxX(textfield.frame), CGRectGetMinY(textfield.frame), 120*_Scale, CGRectGetHeight(textfield.frame));
    ...
    submit.frame=CGRectMake((ScreenWidth-400*_Scale)/2,CGRectGetMaxY(textfield_email.frame)+50*_Scale, 400*_Scale, (400*_Scale*60.0f)/390.0f);
...
//textfield_nickname:i==1?textfield_password:textfield_email;
    ...
    else
    {
        ...
        [str appendString:@"&cell="];
        [str appendString:[_dict objectForKey:@"tel"]];
        [str appendString:@"&code="];
        [str appendString:[_dict objectForKey:@"code"]];
//
...
#pragma mark-发通知刷新发现美校
        [[NSNotificationCenter defaultCenter] postNotificationName:@"changeFound" object:nil];
            ...
            [defaults setObject:[__dict objectForKey:@"ease_mob_username"] forKey:@"chatname"];
            [defaults setObject:[__dict objectForKey:@"ease_mob_password"] forKey:@"chatpassword"];
            [defaults setObject:islogin forKey:@"islogin"];
            //将username存入defaults中
            [defaults setObject:textfield_email.text forKey:@"tel"];
            [defaults setObject:[__dict objectForKey:@"username"] forKey:@"username"];
            [defaults setObject:[__dict objectForKey:@"token"] forKey:@"token"];
            [defaults setObject:[__dict objectForKey:@"is_auth"] forKey:@"is_auth"];
            //将password存入defaults中
            [defaults setObject:textfield_password.text forKey:@"password"];
            //将uid存入defaults中
            [defaults setObject:[__dict objectForKey:@"id"] forKey:@"uid"];
            //取出头像对应的路径
            ...
            {
                imageurl=[__dict objectForKey:@"avatar"];
            }
