...
@property (nonatomic, strong) SouSuoCitiesTableView *tableView;
@property (nonatomic, strong) SouSuoCitiesHeaderView *headerView;
...
-(void)createTableView
{
    WeakSelf(ws);
    if(!_tableView){
        _tableView = [[SouSuoCitiesTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
        ...
        [_tableView setSouSuoCitiesTableViewBlock:^(NSString *type, NSIndexPath *indexPath) {
...
-(void)addHeadViewWhenNoData{
    if(!_headerView){
        WeakSelf(ws);
        _headerView = [[SouSuoCitiesHeaderView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 300)];
...
-(void)setdata:(NSDictionary *)_dict
{
    ...
    {
        //        cn_name
        NSArray *result_arr = [[_dict objectForKey:@"data"] sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
